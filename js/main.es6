$(document).ready(function() {
  ...
  const $searchBarDiv = $('#searchBar');
  const $resetButton = $('#buttonLeft');
  const $submitButton = $('#buttonRight');
  const $surpriseButton = $('#buttonTop');
  const $searchFieldInput = $('#searchField');

  const $resultsDiv = $('div[id=results]');

  const $queryButtonsDiv = $('div[id="queryButtonsDiv"]');
  const $backButton = $('#backButton');
  const $nextButton = $('#nextButton');

  const $pageCounterDiv = $('#pageCounter');
  ...
  function wikiResponse(wikiData) {
    //Function variable(s)
    const _wikiResults = wikiData.query.search;
    ...
    for (let result of _wikiResults) {
      //Loop variables(s)
      const _wikiPageTitle = result.title.replace(" ", "_");
  ...
  function surpriseWikiResponse(surpriseWikiData) {
    //Function variable(s)
    const _surpriseWikiResults = surpriseWikiData.query.random[0].title;
    const _surpriseWikiPageTitle = _surpriseWikiResults.replace(" ", "_");

    //MULTIPLE STATEMENTS
    //STATEMENT: If the random page returned includes one or more of these in the title...
    if (_surpriseWikiPageTitle.startsWith("Category:") ||
        _surpriseWikiPageTitle.startsWith("File:") ||
        _surpriseWikiPageTitle.startsWith("Portal:") ||
        _surpriseWikiPageTitle.startsWith("Talk:") ||
        _surpriseWikiPageTitle.startsWith("Template:") ||
        _surpriseWikiPageTitle.startsWith("User:") ||
        _surpriseWikiPageTitle.startsWith("Wikipedia:") ||
        _surpriseWikiPageTitle.includes("(disambiguation)") ||
        _surpriseWikiPageTitle.includes("_talk:")) {
      //Resend AJAX request
      surpriseAJAXRequest();
  ...
  $submitButton.on('click', function() {
    ...
    //STATEMENT: Else search field has valid text
      ...
      //Function variable(s)
      const _userData = $searchFieldInput.val();
      const _wikiURL = `https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${_userData}&srwhat=text&srprop=timestamp|snippet&sroffset=0&continue=&format=json`;
  ...
  $queryButtonsDiv.children().on('click', function(evt) {
    ...
    //Function variable(s)
    const _offsetWikiURL = `https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${offsetUserData}&srwhat=text&srprop=timestamp|snippet&sroffset=${offsetCounter}&continue=&format=json`;
